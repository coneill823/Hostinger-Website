...
(function () {
  ...
  var particles = [];
  var LINK_DIST = 130;
  var ACCENT = "109, 184, 255";

  function count() { return Math.min(110, Math.round((w * h) / 16000)); }

  function makeParticle() {
    return {
      x: Math.random() * w, y: Math.random() * h,
      vx: (Math.random() - 0.5) * 0.22, vy: (Math.random() - 0.5) * 0.22,
      r: Math.random() * 1.6 + 0.6, a: Math.random() * 0.5 + 0.35
    };
  }
  ...
  function draw() {
    ...
    for (var k = 0; k < particles.length; k++) {
      ...
      ctx.fillStyle = "rgba(" + ACCENT + "," + s.a + ")"; ctx.fill();
    }
  }
